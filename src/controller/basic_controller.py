...
class BasicController(Controller):
    ...
    def get_action(self, vstate: VehicleState, inference: Inference, ds: Decimal) -> Action:
        ...
        print('inference.alpha[i]', inference.alpha[i])
        # p_obstacle_less_than_critical = inference.alpha[i]*d_critical
        # psi_d = [a*ds for a in inference.alpha[:i]]
        # p_obstacle_less_than_critical = sum(psi_d)
        p_obstacle_less_than_critical = sum(inference.alpha[:i])
        print('p_obstacle_less_than_critical', p_obstacle_less_than_critical)
        if float(p_obstacle_less_than_critical) > float(self.prob_threshold):
            a = self.vs.a_min
            print("BRAKEBRAKEBRAKEB")
